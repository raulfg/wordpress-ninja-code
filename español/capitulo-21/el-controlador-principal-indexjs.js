...
filterBtns.forEach( btn => {
    btn.addEventListener( 'click', () => {
        ...
        // Actualizar la URL sin recargar (para compartir el filtro)
...
// Inicializar
if ( grid ) {
    // Si hay proyectos ya renderizados en el servidor (SSR), no recargar
    const existingCards = grid.querySelectorAll( '.portfolio-card' );
    if ( existingCards.length === 0 ) {
        loadPortfolio();
    } else {
        // Ajustar el contador de página si el servidor ya renderizó proyectos

...
class Portfolio_Controller extends \WP_REST_Controller
{
    ...
    {
        // Colección: GET /wp-json/ninjatheme/v1/portfolio
        register_rest_route(
            $this->namespace,
            '/' . $this->rest_base,
            [
        ...
        // Ítem individual: GET /wp-json/ninjatheme/v1/portfolio/{id}
        register_rest_route(
            $this->namespace,
    ...
    public function get_item( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error
    {
        ...
        if ( ! $post ) {
            return new \WP_Error( 'rest_not_found', 'Proyecto no encontrado.', [ 'status' => 404 ] );
        }
    ...
    /**
     * Transforma un WP_Post en el array de respuesta.
     * Separar este método facilita reutilizarlo en get_items y get_item.
     */
    public function prepare_item_for_response( \WP_Post $post ): array
    {
        return [
            ...
            'year'        => (int) get_post_meta( $post->ID, '_año_proyecto', true ) ?: null,
        ];
    }

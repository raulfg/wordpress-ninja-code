...
class HubSpotCRM {
     ...
     *
     * @param array $data  Contact data: email, firstname, lastname, phone, message.
     * @return int|WP_Error HubSpot contact ID or error.
     */
    public function upsert_contact( array $data ): int|\WP_Error {
        ...
        if ( empty( $properties['email'] ) ) {
            return new \WP_Error( 'invalid_email', 'Email required to create the contact.' );
        }
        ...
        // 409 = contact already exists; retrieve the existing ID
        if ( 409 === $code ) {
            return $this->get_contact_id_by_email( $properties['email'] );
        }

        if ( $code < 200 || $code >= 300 ) {
            $message = $body['message'] ?? 'Unknown HubSpot error';
    ...
    /**
     * Gets the ID of an existing contact by email.
     */
    private function get_contact_id_by_email( string $email ): int|\WP_Error {
        ...
        $body = json_decode( wp_remote_retrieve_body( $response ), true );
        return isset( $body['id'] ) ? (int) $body['id'] : new \WP_Error( 'not_found', 'Contact not found.' );
    }

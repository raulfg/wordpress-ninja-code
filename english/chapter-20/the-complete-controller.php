...
class PortfolioController extends \WP_REST_Controller
{
        ...
        register_rest_route( $this->namespace, '/' . $this->rest_base . '/(?P<id>\d+)', [
            [
                ...
                'args'                => [
                    ...
                    '_embed' => [
                        'description' => 'Embed related resources (thumbnail).',
                        'type'        => 'boolean',
                        'default'     => false,
                    ],
                ],
    ...
    public function get_item_permissions_check( \WP_REST_Request $request ): bool|\WP_Error
    {
        ...
        if ( ! $post || 'portfolio' !== $post->post_type ) {
            return new \WP_Error(
                'rest_not_found',
                __( 'Project not found.', 'ninja-portfolio' ),
                [ 'status' => 404 ]
            );
        }
        ...
        if ( 'publish' !== $post->post_status && ! current_user_can( 'edit_post', $post->ID ) ) {
            return new \WP_Error(
                'rest_forbidden',
                __( 'You do not have permission to view this project.', 'ninja-portfolio' ),
                [ 'status' => 403 ]
            );
        }
    ...
    public function get_items( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error
    {
        ...
        $args = [
            ...
            'no_found_rows'  => false, // We need the total for pagination
        ];
    ...
    public function get_item( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error
    {
        ...
        if ( ! $post ) {
            return new \WP_Error( 'rest_not_found', __( 'Project not found.', 'ninja-portfolio' ), [ 'status' => 404 ] );
        }

add_shortcode( 'ninja_member_dashboard', function(): string {
    if ( ! is_user_logged_in() ) {
        return sprintf(
            '<p>%s <a href="%s">%s</a></p>',
            esc_html__( 'To view your dashboard, you must', 'ninja-membership' ),
            esc_url( wp_login_url( get_permalink() ) ),
            esc_html__( 'log in', 'ninja-membership' )
        );
    }
        ...
        <section class="membership-status">
            ...
            <?php if ( $membership ) : ?>
                ...
                <a href="<?php echo esc_url( ninja_get_billing_portal_url( $user_id ) ); ?>" class="button">
                    <?php esc_html_e( 'Manage subscription', 'ninja-membership' ); ?>
                </a>
            <?php else : ?>
                <p><?php esc_html_e( 'You do not have an active membership.', 'ninja-membership' ); ?></p>
                <a href="<?php echo esc_url( ninja_get_plans_page_url() ); ?>" class="button button-primary">
                    <?php esc_html_e( 'View plans', 'ninja-membership' ); ?>
                </a>
            <?php endif; ?>
        </section>
    </div>
